...
\begin{enumerate}
    \item Can be relative or absolute
\end{enumerate}
\begin{sectionbox}
...
\begin{enumerate}
    \item Sometimes the attacker can control the pointer value passed to free.
...
void arbitrary_free(struct object *obj)
{
...
\textbf{Logic Error}
\begin{enumerate}
    \item Patched by Apple ... CVE
    \item Bad code merge or copy/paste
    \item Allows attacker to ...
\end{enumerate}
...
void refcount()
{
    // overflow or mis-matching counting (multiple puts for every get, or multiple gets for every put)
}
\end{lstlisting}
\end{sectionbox}
