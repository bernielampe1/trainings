...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Get a feel for the code style and find places the style is violated. This can tell you if multiple authors are writing and merging code and opens up chances for mis-communications.
...
\begin{enumerate}
	...
	\item Program input vectors are:
 		...
 		\end{enumerate}
	\item An important thing to keep in mind when auditing is what options where used at compile time. This will help you ignore code not compiled into the target.
...
\begin{enumerate}
	...
	\item One interesting strategy that continues to be exploitable is if you can write bytes to the stack at predictiable offsets. If so, then overwriting pointers on the stack is useful to shift stack frames pointers or pointers to objects onthe stack up or down into perhaps attacker controlled areas.
...
\begin{enumerate}
    \item Format strings are not found often, because they are easy for compilers to programmatically check for. The compiler can easily check if the format string is not a constant string and can also check if the number of parameters in the format string matches the number and type of arguments. This prevents the class of bug entirely.
...
\begin{enumerate}
    ...
    \item Most libc implementation will catch this because it's a simple check.
\end{enumerate}
\begin{sectionbox}
...
\begin{enumerate}
    ...
    \item Critical aspects of exploitability of these bugs are the ability to spray or groom the heap to replace the memory with attacker controlle data. Then having that attacker controlled way used in an interesting way.
\end{enumerate}
\begin{sectionbox}
...
\begin{enumerate}
    \item Type confusions are more often found in languages where polymorphism is built in such as C++.
    \item However, type confusion can occur where type casting occurs. Look out for explicit cast operators in C and reinterpret\_type() calls in C++. These circumvent type safety semantic checking in the compilers.
\end{enumerate}
\begin{sectionbox}
...
\begin{enumerate}
    \item The most infamous information disclosure is HeartBleed (CVE-2014-0346) in OpenSSL which would allow a remote attacker to dump heap data from a server process.
    \item It was called HeartBleed because the heat beat (hello I'm alive) packet in the TLS was poorly implemented in the library.
    \item This is an example of when a bug isn't found because the attack surface goes unnotice. Typically new attack surfaces (no matter how simple) can habor bugs.
\end{enumerate}
\begin{sectionbox}
\begin{lstlisting}[language=C, caption={Info Leak}]
// based on CVE-2014-0346, the attacker controls payload
...
\begin{enumerate}
    \item Race conditions, also calls TOUTOC (for Time-of-Use,Time-of-Check), are bugs in concurrent programs (multi-threading or multi-process) where shared state is not property synchronized with the use of locking mechanisms.
    \item These are hard to check for using static analysis techniques such as source code auditing.
    \item When you see locks being used or state being shared, ask "What memory and variables are shared?" and "What synchronization mechcanisms are used to serialize access (spin locks, event locks, reader/writer locks, semaphores)?"
    \item Some race conditions are very hard to hit. Also look for global locks which will be in higher function calls. These are sometimes used as band-aids for the developers when run time is not an issue. Past Linux versions and Python2 used global locks at runtime.
    \item Some race conditions are very hard to hit. Also look for global locks which will be in higher function calls. These are sometimes used as band-aids for the developers when run time is not an issue. Past Linux versions and Python2 used global locks at runtime.
...
\textbf{REF Counting}
\begin{enumerate}
    \item Reference counting can be viewed as a memory allocator with amortized work. All references to a single object use the same underlying sturcture/object memory. Allocation happens during initialization (RAII) and free'ing happens when reference count goes to zero.
    \begin{enumerate}
        \item A put operations is decrementing (free'ing) a ref count. When refcount goes to zero, then memory is free'd.
    ...
    \begin{enumerate}
        ...
        \item If an attacker can invoke an extra get, then the attacker can integer overflow the refcount to zero and achieve a UAF.
    \end{enumerate}
\end{enumerate}
\begin{sectionbox}
...
{
    if (argc != 2) return -1;
    uint32_t num_to_alloc = strtol(argv[1], NULL, 10);
...
\begin{enumerate}
    ...
    \item As a rule of thumb, the compiler emit assembly code so that the integers are promoted to the widest type and then compared.
    \item Integer ranges:
        \begin{enumerate}
            \item signed char: -127 to 127
            \item unsigned char: 0 to 255
            \item signed short: -32767 to 32767
            \item unsigned short: 0 to 65535
            \item signed int: -2147483647 to 2147483647 */
        \end{enumerate}
\end{enumerate}
